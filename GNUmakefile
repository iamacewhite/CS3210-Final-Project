...
LAB ?= $(LAB)   # Run a grading script for a previous lab with, for example, "make grade LAB=4". Defaults to what is in conf/lab.mk.
LABSETUP ?= ./
...
.PRECIOUS: %.o $(OBJDIR)/boot/%.o $(OBJDIR)/kern/%.o \
	   $(OBJDIR)/lib/%.o $(OBJDIR)/fs/%.o $(OBJDIR)/page/%.o $(OBJDIR)/net/%.o \
	   $(OBJDIR)/user/%.o
...
include fs/Makefrag
include page/Makefrag


CPUS ?= 1

QEMUOPTS = -hda $(OBJDIR)/kern/kernel.img -serial mon:stdio -gdb tcp::$(GDBPORT) -snapshot
